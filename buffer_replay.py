...
class ReplayWindow:
    ...
    def __init__(self, history: GameHistory,allfen,moves_dict):
        self.actions = []
        ...
        self.screen = pygame.display.set_mode((self.width*2.3, self.height))
        ...
        x, y = self.text_coords_below(chess.D1)
        self.buttons.append(Button(2*x, y, self.square_size / 2, self.square_size / 4, self.font, 'o',
                                   onclick=self.go_buffer))
    ...
    def go_buffer(self):
        self.board.set_fen(self.actions[self.action_index+2]['fen'])
        for square in chess.SQUARES:
            color = self.LIGHT_COLOR if square in self.LIGHT_SQUARES else self.DARK_COLOR
            s=tuple([2*x for x in self.square_rect(square)])
            print(s)
            pygame.draw.rect(self.background, color,s)

            piece = self.board.piece_at(square)
            if piece is not None:
                image = self.image_by_piece[piece]
                self.background.blit(image,s)
    ...
    def draw(self):
        ...
        pygame.draw.line(self.background, (200, 200, 200), (0, self.square_size * 10),
    ...
    def draw_board(self):
        ...
        if self.action_index is None:
            ...
            ind=str(int(self.action_index/4))
            for i in range(min(len(self.allfen[ind]),5)):
                # self.board.set_fen(self.topkfen[self.action_index][i])
                x=sorted(self.allfen[ind],reverse=True)
                self.board.set_fen(x[i][1])
                cnt=0;s_left=0;s_top=0;f=0.4
                for square in chess.SQUARES:
                    color = self.LIGHT_COLOR if square in self.LIGHT_SQUARES else self.DARK_COLOR
                    
                    s=tuple(self.square_rect(square))
                    if cnt==0:
                        s_left=s[0]
                        s_top=s[1]
                    s=tuple([s_left+8*self.square_size+((cnt%8)+9*int(i%3))*f*self.square_size,f*self.square_size*(11*int(i/3)+int(cnt/8)),s[2]*f,s[3]*f])
                    cnt+=1
                    pygame.draw.rect(self.background, color,s)

                    text = 'TopBoard:{}'.format(np.round(x[i][0],2))
                    text_width, text_height = self.font.size(text)
                    self.background.blit(self.font.render(text, True, (0, 0, 0)), (s_left+8*self.square_size+(2+9*int(i%3))*f*self.square_size,f*self.square_size*(11*int(i/3)+int(72/8))))

                    piece = self.board.piece_at(square)
                    if piece is not None:
                        image = self.image_by_piece[piece]
                        self.background.blit(pygame.transform.scale(image,(64*f,64*f)),s)

            for i in range(min(len(self.moves_dict[ind]),5)):
                # self.board.set_fen(self.topkfen[self.action_index][i])
                x=sorted(self.moves_dict[ind],reverse=True)
                text = 'TopMove:{}'.format([np.round(x[i][0],2),x[i][1]])
                text_width, text_height = self.font.size(text)
                self.background.blit(self.font.render(text, True, (0, 0, 0)),(1200,350+30*i) )


            cur_move_score=[];cur_board_prob=[]
            for i in self.moves_dict[ind]:
                if self.actions[self.action_index].get('taken_move') and i[1]==str(self.actions[self.action_index]['taken_move']):
                    cur_move_score=[np.round(i[0],2),i[1]]
                    
           
            for i in self.allfen[ind]:
                if self.actions[self.action_index]['fen'].startswith(i[1]):
                    cur_board_prob=np.round(i[0],4)
                    

            text = 'CurMoveScore:{}'.format(cur_move_score)
            text_width, text_height = self.font.size(text)
            self.background.blit(self.font.render(text, True, (0, 0, 0)),(1200,350+30*6) )

            text = 'CurBoardProb:{}'.format(cur_board_prob)
            text_width, text_height = self.font.size(text)
            self.background.blit(self.font.render(text, True, (0, 0, 0)),(1200,350+30*7) )
...
def main():
    ...
    f = open('sample_fen.json')
    data_fen = json.load(f)
    f = open('sample_moves.json')
    data_moves = json.load(f)

    print(data_moves)
    #print(sorted(data['2'],reverse=True))
    history = GameHistory.from_file(args.history_path)
    if history.is_empty():
        print('Game History is empty.')
        quit()

    window = ReplayWindow(history,data_fen,data_moves)
